    ...
    <div class="container-fluid">
        <div class="card shadow mb-4">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">Data Outcome:</h6>
            </div>
            <!-- Card Body -->
            <div class="card-body">
                <div class="row mt-3">
                    ...
                    <div class="col-md-7 mt-4">
                        <h7 class="m-0 font-weight-bold">Daily Outcome: </h7><br>
                ...
                <div class="table-responsive service">
                    <table class="table table-bordered table-hover mt-3 text-nowrap css-serial">
                        ...
                        <tbody>
                            @foreach ($outcome as $order=> $item)
                                <tr>
                                    <th scope="row">{{ ($outcome->currentPage() - 1) * $outcome->perPage() + $order+ 1 }}</th>
                                    <td>{{ 'Rp ' . number_format($item->total , 0, ',', '.') }}</td>
                                    <td class="catatan">{{ $item->keterangan }}</td>
                                    <td>{{ \Carbon\Carbon::parse($item->tanggal)->format('d-m-Y') }}</td>
                                    <td>                                                                                
                                        <a href="{{ route('outcome.edit', $item->id) }}" class="btn btn-success btn-sm">Edit</a>                                            
                                        
                                        <form action="{{ route('outcome.destroy', $item->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="modal fade" id="exportModal" tabindex="-1" role="dialog" aria-labelledby="exportModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('export_outcomes') }}" method="POST">
                            @csrf
                            <div class="form-group">
